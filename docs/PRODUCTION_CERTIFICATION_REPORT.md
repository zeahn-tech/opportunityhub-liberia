...
> **Update — September 9-10, 2026 (Phase 3, dbClient → Supabase migration — IN PROGRESS, Services 1-4 of 9 complete)**: This phase moves each domain service off `src/db/dbClient.ts`'s local/localStorage store and onto real Supabase queries under RLS, one service at a time, per `docs/REMEDIATION_PHASES.md`'s ordering. **Service 1 (organizationService / memberships)**, **Service 2 (opportunityService)**, **Service 3 (applicationService)**, and **Service 4 (candidateService)** are complete and proven as of this update — see the "Architecture Status" and "Database Status" notes below, and the full records in `docs/PHASE3_SERVICE1_VERIFICATION.md` through `docs/PHASE3_SERVICE4_VERIFICATION.md`. Services 5–9 (businessService, messagingService, verificationService, trustSafetyService, subscriptionService/notificationService/analyticsService) have **not** been started. Services 2, 3, and 4 are actually **live** in the running application; Service 1 exists, typechecks, is tested, and is proven against live RLS, but its callers are not yet rewired. Service 3's migration fixed a real pre-existing RLS gap (a candidate could self-promote their own application's pipeline stage via direct API call). Service 4's migration fixed an even more fundamental one: the pre-existing RLS let a candidate see only their own profile and let **no one else see any candidate profile at all**, which breaks the core point of a job marketplace — fixed with two `SECURITY DEFINER` RPCs that compute the same conditional column-level redaction (masked identity for 'anonymous' profiles, contact info gated on an actual application relationship) that Postgres RLS alone cannot express, live-proven including the "applying unlocks visibility" rule actually working end-to-end.
...
* **Phase 3 update (September 9-10, 2026, in progress)**: `src/services/organizationService.ts`, `src/services/opportunityService.ts`, `src/services/applicationService.ts`, and `src/services/candidateService.ts` now exist as Supabase-backed domain services reading/writing through the anon/user-session client, with Postgres RLS (not client code) as the authorization boundary. opportunityService, applicationService, and candidateService are live in the running app; organizationService exists and is proven but its callers are not yet rewired. All four are independently verified (`docs/PHASE3_SERVICE1_VERIFICATION.md` through `docs/PHASE3_SERVICE4_VERIFICATION.md`). candidateService's migration also introduced two `SECURITY DEFINER` RPCs (`get_public_candidate_profile`, `search_candidate_profiles`) as the only path to viewing anyone else's profile — the raw table has no third-party SELECT policy at all, since candidate visibility requires conditional column-level redaction RLS alone can't express (the same pattern `businessService`, not yet migrated, will need for confidential financials). The architecture is therefore mid-migration: four real Supabase-backed services exist (three fully live, one proven-but-not-wired), the other 5 planned services and most existing callers still point at `dbClient.ts`. Do not read this bullet as "Architecture Status: 100% Fully Implemented" being re-certified — that status line above describes the September 7 audit and has not been re-verified against the current, partially-migrated state.
...
| businessService, messagingService, verificationService, trustSafetyService, subscriptionService, notificationService, analyticsService | **Not started** | Still 100% `dbClient.ts`/`localStorage`, unchanged from the September 7/8 state described below. |
