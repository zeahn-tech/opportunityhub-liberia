...
> **Update — September 9-11, 2026 (Phase 3, dbClient → Supabase migration — IN PROGRESS, Services 1-8 of 9 complete)**: This phase moves each domain service off `src/db/dbClient.ts`'s local/localStorage store and onto real Supabase queries under RLS, one service at a time, per `docs/REMEDIATION_PHASES.md`'s ordering. **Services 1 through 8** (organizationService, opportunityService, applicationService, candidateService, businessService, messagingService, verificationService, trustSafetyService) are complete and proven as of this update — see the "Architecture Status" and "Database Status" notes below, and the full records in `docs/PHASE3_SERVICE1_VERIFICATION.md` through `docs/PHASE3_SERVICE8_VERIFICATION.md`. Only Service 9 (subscriptionService/notificationService/analyticsService) remains. Services 2, 3, 4, 6, 7, and 8 are live in the running application; Service 1 exists and is proven but its callers aren't rewired yet; Service 5 (businessService) is live but has a flagged UI follow-up. Service 8 (trustSafetyService) built its own schema from scratch (the second time this phase has done so, after messaging/Service 6) covering a separate, richer verification-request system alongside content reports, account restrictions, and suspicious-activity detection — and moved a server-side-only-computable invariant (the multi-report auto-quarantine threshold, which requires counting reports across all reporters, not just the caller's own visible rows) into a SECURITY DEFINER RPC. Live testing also required rewiring `TrustSafetyAdminCenter.tsx`'s admin dashboard, which had been reading trust & safety data directly from `dbClient.ts` with no mount-time refresh at all.
>
> **Note on push status**: as of this update, all Phase 3 work through Service 8 is committed locally but GitHub push access remains interrupted (the previously-provided access token was revoked mid-phase, the expected/correct outcome of that token having been exposed earlier in this work — see the chat history around Service 4/5). Local commits are intact and will be pushed once repository write access is re-established.
...
* **Phase 3 update (September 9-11, 2026, in progress)**: `src/services/organizationService.ts`, `src/services/opportunityService.ts`, `src/services/applicationService.ts`, `src/services/candidateService.ts`, `src/services/businessService.ts`, `src/services/messagingService.ts`, `src/services/verificationService.ts`, and `src/services/trustSafetyService.ts` now exist as Supabase-backed domain services reading/writing through the anon/user-session client, with Postgres RLS (not client code) as the authorization boundary. All but organizationService are live in the running app; organizationService exists and is proven but its callers are not yet rewired. All eight are independently verified (`docs/PHASE3_SERVICE1_VERIFICATION.md` through `docs/PHASE3_SERVICE8_VERIFICATION.md`). trustSafetyService's migration reused `is_platform_admin()` (introduced by verificationService, Service 7) across five new admin-gated RPCs, and required rewiring `TrustSafetyAdminCenter.tsx` itself, which had been reading trust & safety data directly from `dbClient.ts` with no mount-time refresh at all. The architecture is therefore mid-migration: eight real Supabase-backed services exist (seven live end-to-end, one proven-but-not-wired), only Service 9 and most existing callers still point at `dbClient.ts`. Do not read this bullet as "Architecture Status: 100% Fully Implemented" being re-certified — that status line above describes the September 7 audit and has not been re-verified against the current, partially-migrated state.
...
| subscriptionService, notificationService, analyticsService | **Not started** | Still 100% `dbClient.ts`/`localStorage`, unchanged from the September 7/8 state described below. |
