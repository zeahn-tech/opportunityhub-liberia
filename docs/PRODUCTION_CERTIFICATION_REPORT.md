...
> **Update — September 9-11, 2026 (Phase 3, dbClient → Supabase migration — IN PROGRESS, Services 1-7 of 9 complete)**: This phase moves each domain service off `src/db/dbClient.ts`'s local/localStorage store and onto real Supabase queries under RLS, one service at a time, per `docs/REMEDIATION_PHASES.md`'s ordering. **Services 1 through 7** (organizationService, opportunityService, applicationService, candidateService, businessService, messagingService, verificationService) are complete and proven as of this update — see the "Architecture Status" and "Database Status" notes below, and the full records in `docs/PHASE3_SERVICE1_VERIFICATION.md` through `docs/PHASE3_SERVICE7_VERIFICATION.md`. Services 8–9 (trustSafetyService, subscriptionService/notificationService/analyticsService) have **not** been started. Services 2, 3, 4, 6, and 7 are live in the running application; Service 1 exists and is proven but its callers aren't rewired yet; Service 5 (businessService) is live but has a flagged UI follow-up. Service 7 (verificationService) fixed two real gaps: the platform-admin review queue was previously unreachable through RLS at all (no policy let anyone outside an org see its own pending requests except that org itself), and approving a request never actually updated the organization's verification badge in the old implementation — verification was purely cosmetic. Both fixed with a new `is_platform_admin()` helper and an atomic decision RPC, live-verified including a cross-service composition check: approving a request correctly triggered Service 1's `sync_organization_verified_flag` trigger automatically.
>
> **Note on push status**: as of this update, all Phase 3 work through Service 7 is committed locally but GitHub push access has been temporarily interrupted (the previously-provided access token was revoked mid-phase, which is the expected/correct outcome of that token having been exposed — see the chat history around Service 4/5 for that exchange). Local commits are intact and will be pushed once repository write access is re-established.
...
* **Phase 3 update (September 9-11, 2026, in progress)**: `src/services/organizationService.ts`, `src/services/opportunityService.ts`, `src/services/applicationService.ts`, `src/services/candidateService.ts`, `src/services/businessService.ts`, `src/services/messagingService.ts`, and `src/services/verificationService.ts` now exist as Supabase-backed domain services reading/writing through the anon/user-session client, with Postgres RLS (not client code) as the authorization boundary. All but organizationService are live in the running app; organizationService exists and is proven but its callers are not yet rewired. All seven are independently verified (`docs/PHASE3_SERVICE1_VERIFICATION.md` through `docs/PHASE3_SERVICE7_VERIFICATION.md`). verificationService's migration introduced `is_platform_admin()`, the first reusable "is this user a platform admin" RLS helper in this phase — trustSafetyService (Service 8) will need the same concept. The architecture is therefore mid-migration: seven real Supabase-backed services exist (six live end-to-end, one proven-but-not-wired), the other 2 planned services and most existing callers still point at `dbClient.ts`. Do not read this bullet as "Architecture Status: 100% Fully Implemented" being re-certified — that status line above describes the September 7 audit and has not been re-verified against the current, partially-migrated state.
...
| trustSafetyService, subscriptionService, notificationService, analyticsService | **Not started** | Still 100% `dbClient.ts`/`localStorage`, unchanged from the September 7/8 state described below. |
