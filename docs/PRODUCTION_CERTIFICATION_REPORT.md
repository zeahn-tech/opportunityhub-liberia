...
> **Update — September 9-10, 2026 (Phase 3, dbClient → Supabase migration — IN PROGRESS, Services 1-3 of 9 complete)**: This phase moves each domain service off `src/db/dbClient.ts`'s local/localStorage store and onto real Supabase queries under RLS, one service at a time, per `docs/REMEDIATION_PHASES.md`'s ordering. **Service 1 (organizationService / memberships)**, **Service 2 (opportunityService)**, and **Service 3 (applicationService)** are complete and proven as of this update — see the "Architecture Status" and "Database Status" notes below, and the full records in `docs/PHASE3_SERVICE1_VERIFICATION.md` / `docs/PHASE3_SERVICE2_VERIFICATION.md` / `docs/PHASE3_SERVICE3_VERIFICATION.md`. Services 4–9 (candidateService, businessService, messagingService, verificationService, trustSafetyService, subscriptionService/notificationService/analyticsService) have **not** been started — those domains still run entirely on `dbClient.ts`/`localStorage`. Services 2 and 3 are actually **live** in the running application (`src/App.tsx`'s opportunity and application listing, creation, and all lifecycle mutations now hit Supabase for real); Service 1 exists, typechecks, is tested, and is proven against live RLS, but its callers (`authService.ts`, `permissionEngine.ts`, 3 components) are not yet rewired — see the verification docs for why that's a deliberate stopping point per service, not an oversight. Service 3's migration also fixed a real pre-existing RLS gap (not something this phase introduced): the "candidates can withdraw applications" policy had no column restriction, so a candidate could previously self-promote their own application's pipeline stage via a direct API call — closed with a new database trigger, live-proven against the actual vulnerability, not just the schema.
...
* **Phase 3 update (September 9-10, 2026, in progress)**: `src/services/organizationService.ts`, `src/services/opportunityService.ts`, and `src/services/applicationService.ts` now exist as Supabase-backed domain services reading/writing through the anon/user-session client, with Postgres RLS (not client code) as the authorization boundary. opportunityService and applicationService are live in the running app (`src/App.tsx`'s opportunity/application listing, creation, and lifecycle mutations); organizationService exists and is proven but its callers are not yet rewired. All three are independently verified (`docs/PHASE3_SERVICE1_VERIFICATION.md`, `docs/PHASE3_SERVICE2_VERIFICATION.md`, `docs/PHASE3_SERVICE3_VERIFICATION.md`). The architecture is therefore mid-migration: three real Supabase-backed services exist (two fully live, one proven-but-not-wired), the other 6 planned services and most existing callers still point at `dbClient.ts`. Do not read this bullet as "Architecture Status: 100% Fully Implemented" being re-certified — that status line above describes the September 7 audit and has not been re-verified against the current, partially-migrated state.
...
| candidateService, businessService, messagingService, verificationService, trustSafetyService, subscriptionService, notificationService, analyticsService | **Not started** | Still 100% `dbClient.ts`/`localStorage`, unchanged from the September 7/8 state described below. |
