...
> **Update — September 9-10, 2026 (Phase 3, dbClient → Supabase migration — IN PROGRESS, Services 1-2 of 9 complete)**: This phase moves each domain service off `src/db/dbClient.ts`'s local/localStorage store and onto real Supabase queries under RLS, one service at a time, per `docs/REMEDIATION_PHASES.md`'s ordering. **Service 1 (organizationService / memberships)** and **Service 2 (opportunityService)** are complete and proven as of this update — see the "Architecture Status" and "Database Status" notes below, and the full records in `docs/PHASE3_SERVICE1_VERIFICATION.md` / `docs/PHASE3_SERVICE2_VERIFICATION.md`. Services 3–9 (applicationService, candidateService, businessService, messagingService, verificationService, trustSafetyService, subscriptionService/notificationService/analyticsService) have **not** been started — those domains still run entirely on `dbClient.ts`/`localStorage`. Service 2 is the first one actually **live** in the running application (opportunity listing, creation, and all lifecycle mutations in `src/App.tsx` now hit Supabase for real); Service 1 exists, typechecks, is tested, and is proven against live RLS, but its callers (`authService.ts`, `permissionEngine.ts`, 3 components) are not yet rewired — see the verification docs for why that's a deliberate stopping point per service, not an oversight.
...
* **Phase 3 update (September 9-10, 2026, in progress)**: `src/services/organizationService.ts` and `src/services/opportunityService.ts` now exist as Supabase-backed domain services reading/writing through the anon/user-session client, with Postgres RLS (not client code) as the authorization boundary. opportunityService is live in the running app (`src/App.tsx`'s opportunity listing, creation, and lifecycle mutations); organizationService exists and is proven but its callers are not yet rewired. Both are independently verified (`docs/PHASE3_SERVICE1_VERIFICATION.md`, `docs/PHASE3_SERVICE2_VERIFICATION.md`). The architecture is therefore mid-migration: two real Supabase-backed services exist (one fully live, one proven-but-not-wired), the other 7 planned services and most existing callers still point at `dbClient.ts`. Do not read this bullet as "Architecture Status: 100% Fully Implemented" being re-certified — that status line above describes the September 7 audit and has not been re-verified against the current, partially-migrated state.
...
| applicationService, candidateService, businessService, messagingService, verificationService, trustSafetyService, subscriptionService, notificationService, analyticsService | **Not started** | Still 100% `dbClient.ts`/`localStorage`, unchanged from the September 7/8 state described below. |
